...
public final class SecurityRequestPostProcessors {
     ...
     */
    public static UserDetailsRequestPostProcessor createUserDetailsRequestPostProcessor(String username) {
    ...
    public final static class UserDetailsRequestPostProcessor
         ...
         * can be problematic if multiple {@link UserDetailsService} beans are declared.
         */
        public UserDetailsRequestPostProcessor userDetailsService(UserDetailsService userDetailsService) {
            this.userDetailsService = userDetailsService;
            return this;
        }
        
        public MockHttpServletRequest postProcessRequest(MockHttpServletRequest request) {
            OpenIDAuthenticationToken authentication = authentication(request.getServletContext());
            save(authentication,request);
            return request;
        }

        private OpenIDAuthenticationToken authentication(ServletContext servletContext) {
...
//        }
        ...
        private OpenIDAuthenticationToken openIdAuthentication(ServletContext servletContext) {
            ApplicationContext context = WebApplicationContextUtils.getRequiredWebApplicationContext(servletContext);
...
//                    userDetails, userDetails.getPassword(), userDetails.getAuthorities());
        ...
        private UserDetailsService userDetailsService(ApplicationContext context) {
            if(this.userDetailsServiceBeanId == null) {
                return context.getBean("openIdUserDetailsService", UserDetailsService.class);
            }
            return context.getBean(this.userDetailsServiceBeanId, UserDetailsService.class);
        }
         */
